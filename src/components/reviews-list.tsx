    ...
    <>
      ...
      <div className="flex items-center gap-1 border-b border-[#1f1f23]">
        {TABS.map((t) => {
          ...
          const count = counts[t.key as keyof typeof counts] ?? counts.all;
          return (
            <button
              key={t.key}
              onClick={() => setActiveTab(t.key)}
              className={`flex items-center gap-1.5 px-3 py-2 text-sm cursor-pointer border-b-2 -mb-px transition-all duration-150 ${
                isActive
      ...
      {filtered.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
          ...
          <p className="text-zinc-300 font-medium">
            {activeTab === "all" ? "No reviews yet" : `No ${STATUS_LABEL[activeTab] ?? activeTab} reviews`}
          </p>
          <p className="text-zinc-600 text-sm max-w-xs">
            {activeTab === "all"
              ? "Connect a repo and open a Pull Request — the AI review will appear here automatically."
              : "Reviews in this category will show up here."}
          </p>
        </div>
      ) : (
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        {warnings > 0 && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] border bg-yellow-500/10 text-yellow-300 border-yellow-500/30">
                            <span className="w-1.5 h-1.5 rounded-full bg-yellow-400" />{warnings}
                          </span>
                        )}
                        {suggestions > 0 && (
                          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] border bg-emerald-500/10 text-emerald-300 border-emerald-500/30">
                            <span className="w-1.5 h-1.5 rounded-full bg-emerald-400" />{suggestions}
                          </span>
                        )}
                      </div>
                    )}
